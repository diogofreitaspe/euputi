...
workon euputi  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development euputi npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop euputi
    docker rm euputi
    docker run --name euputi -d --env-file /home/ubuntu/euputi.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/euputi:/dkdata \
        euputi start_web.sh
}
...
echo_green "Welcome to euputi's dev env"
